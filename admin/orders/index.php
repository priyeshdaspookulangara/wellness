...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] == 'update_status') {
    ...
    // Redirect to same page to prevent form resubmission, preserving filter
    $redirect_url = SITE_URL . "admin/orders/"; // Corrected redirect base
    if (isset($_GET['filter_status']) && !empty($_GET['filter_status'])) {
        $redirect_url .= "?filter_status=" . urlencode($_GET['filter_status']);
    }
...
<div class="mb-3">
    <form action="<?php echo SITE_URL; ?>admin/orders/" method="GET" class="form-inline"> <!-- Corrected form action -->
        ...
        <button type="submit" class="btn btn-primary">Filter</button>
         <?php if (!empty($filter_status)): ?>
            <a href="<?php echo SITE_URL; ?>admin/orders/" class="btn btn-outline-secondary ms-2">Clear Filter</a> <!-- Corrected link -->
        <?php endif; ?>
    </form>
</div>
            ...
            <?php if (!empty($orders)): ?>
                <?php foreach ($orders as $order): ?>
                    <tr>
                        ...
                        <td>
                            <form action="<?php echo SITE_URL; ?>admin/orders/<?php echo !empty($filter_status) ? '?filter_status='.$filter_status : ''; ?>" method="POST" class="form-inline"> <!-- Corrected form action -->
                                <input type="hidden" name="action" value="update_status">
                                <input type="hidden" name="order_id" value="<?php echo $order['id']; ?>">
                                <select name="status" class="form-control form-control-sm form-select" onchange="this.form.submit()">
                        ...
                        <td class="action-buttons">
                            <a href="../order_view/?id=<?php echo $order['id']; ?>" class="btn btn-sm btn-info" title="View Details"><i class="fas fa-eye"></i></a> <!-- Corrected link -->
                        </td>
                    </tr>
                <?php endforeach; ?>
            <?php else: ?>
