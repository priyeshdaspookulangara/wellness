...
if (!$user_id_to_view) {
    ...
    header("Location: " . SITE_URL . "admin/users/"); // Corrected redirect
    exit;
}

$page_title = "View/Edit User";
// Breadcrumbs will be set dynamically after fetching user data
...
// Update page title and breadcrumbs with user data
$page_title = "View/Edit User: " . htmlspecialchars($user_data['username']);
$breadcrumbs = [
    ['name' => 'Users', 'link' => SITE_URL . 'admin/users/'],
    ['name' => htmlspecialchars($user_data['username'])]
];
// We need to re-include header if we want breadcrumbs to update with dynamic data
// This is a common issue with simple include systems. A better way is to buffer header output or use a template engine.
// For now, the title in the tab will be generic, but the H1 will be specific.
// Or, we can output header content after fetching data.
// Let's stick to one header include for now. The $page_title in <title> will be generic.

$errors = [];
$success_message = '';

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Handle form submission for updates (logic remains same as before)
    $new_data = [
        ...
        'is_admin' => (isset($_POST['is_admin']) && $user_id_to_view != $_SESSION['user_id']) ? 1 : ($user_data['is_admin'] ? 1:0)
    ];
    ...
    if (empty($errors)) {
        ...
        $sql_update_user = "UPDATE users SET
                                email = '$email_esc', first_name = '$fname_esc', last_name = '$lname_esc',
                                address_street = '$street_esc', address_city = '$city_esc', address_state = '$state_esc',
                                address_zip = '$zip_esc', address_country = '$country_esc', phone_number = '$phone_esc',
                                is_admin = $is_admin_sql, updated_at = NOW()
                            WHERE id = $user_id_to_view";

        if (mysqli_query($conn, $sql_update_user)) {
            $success_message = "User details updated successfully.";
            $result_fetch_user = mysqli_query($conn, $sql_fetch_user); // Re-fetch
            $user_data = mysqli_fetch_assoc($result_fetch_user);
            // Update dynamic page title for H1
            $page_title = "View/Edit User: " . htmlspecialchars($user_data['username']);
    ...
    $user_data = array_merge($user_data, $new_data); // For sticky form
}
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    <h1 class="h2"><?php echo $page_title; // This will now show the updated title ?></h1>
    <a href="<?php echo SITE_URL; ?>admin/users/" class="btn btn-sm btn-outline-secondary">Back to Users</a> <!-- Corrected link -->
</div>
...
<form action="<?php echo SITE_URL; ?>admin/user_view/?id=<?php echo $user_id_to_view; ?>" method="POST"> <!-- Corrected form action -->
    <div class="row">
        <div class="col-md-6">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group mb-3">
                        ...
                        <select class="form-control form-select" id="is_admin" name="is_admin" <?php echo ($user_id_to_view == $_SESSION['user_id']) ? 'disabled' : ''; ?>>
        ...
        <div class="col-md-6">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="row"> <!-- BS5 uses .row for form-row behavior -->
        ...
        <table class="table table-sm table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="<?php echo SITE_URL; ?>admin/orders/order_view/?id=<?php echo $uo['id']; ?>" class="btn btn-xs btn-info">View Order</a></td> <!-- Corrected link -->
                </tr>
                <?php endforeach; ?>
            </tbody>
        </table>
        ...
        <table class="table table-sm table-striped">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td><a href="<?php echo SITE_URL . 'product/?slug=' . htmlspecialchars($liked_prod['slug']); ?>" target="_blank"><?php echo htmlspecialchars($liked_prod['name']); ?></a></td> <!-- Corrected link -->
                    <td><?php echo date("M j, Y, g:i a", strtotime($liked_prod['liked_at'])); ?></td>
                    <td>
                        <a href="<?php echo SITE_URL; ?>admin/products/product_edit/?id=<?php echo $liked_prod['id']; ?>" class="btn btn-xs btn-outline-secondary">View Product</a> <!-- Corrected link -->
                    </td>
                </tr>
                <?php endforeach; ?>
            </tbody>
