...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $product_data['slug'] = trim($_POST['slug'] ?? ''); // Auto-generate if empty?
    ...
    // Image Upload Handling (Simplified - needs more robust implementation)
    $image_file_name = null;
    if (isset($_FILES['image_url_main']) && $_FILES['image_url_main']['error'] == UPLOAD_ERR_OK) {
        $upload_dir = __DIR__ . '/../uploads/'; // Create this directory if it doesn't exist and make it writable
        ...
        if (!in_array(strtolower($file_info['extension']), $allowed_types)) {
            ...
            $image_file_name = null; // Reset on failure
        }
    }


    if (empty($errors)) {
        // Escape all string data
        ...
        if (mysqli_num_rows($res_slug) > 0) {
            ...
            if (mysqli_query($conn, $sql_insert)) {
                ...
                $errors[] = "Failed to add product: " . mysqli_error($conn);
                 // If insert fails and image was uploaded, consider deleting the uploaded image
        ...
        <div class="col-md-4">
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <div class="form-group" id="sale_price_group" style="<?php echo $product_data['is_on_sale'] ? '' : 'display:none;'; ?>">
            ...
            <div class="card mb-3">
                ...
                <div class="card-body">
                    ...
                    <!-- Placeholder for image gallery uploads -->
                </div>
            </div>
             <div class="card mb-3">
...
$(document).ready(function(){
    $('#is_on_sale').change(function(){
        if($(this).is(":checked")) {
            ...
            $('#sale_price').val(''); // Clear sale price if not on sale
        }
    });
});
</script>
