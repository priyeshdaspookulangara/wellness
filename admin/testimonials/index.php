...
$breadcrumbs = [ ['name' => 'Testimonials/Reviews'] ];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['review_id'])) {
    ...
    $redirect_url_admin_testimonials = SITE_URL . "admin/testimonials/"; // Corrected base redirect
    if (isset($_GET['filter_status']) && !empty($_GET['filter_status'])) {
        $redirect_url_admin_testimonials .= "?filter_status=" . urlencode($_GET['filter_status']);
    }
    header("Location: " . $redirect_url_admin_testimonials);
    exit;
}


// Filtering
$filter_status_admin = $_GET['filter_status'] ?? 'pending';
$where_clause_admin = '';
...
// Fetch reviews (logic remains same)
$reviews_admin = [];
$sql_reviews_admin = "SELECT r.id, r.rating, r.comment, r.status, r.created_at,
                             u.username AS user_username, p.name AS product_name, p.slug AS product_slug
                      FROM reviews r
...
<div class="mb-3">
    <form action="<?php echo SITE_URL; ?>admin/testimonials/" method="GET" class="form-inline"> <!-- Corrected form action -->
        <label for="filter_status" class="mr-2">Filter by Status:</label>
        <select name="filter_status" id="filter_status" class="form-control form-select me-2">
        ...
        <button type="submit" class="btn btn-primary">Filter</button>
         <?php if (!empty($filter_status_admin)): ?>
            <a href="<?php echo SITE_URL; ?>admin/testimonials/" class="btn btn-outline-secondary ms-2">Clear Filter / Show Pending</a> <!-- Corrected link -->
        <?php endif; ?>
    </form>
</div>
            ...
            <?php if (!empty($reviews_admin)): ?>
                <?php foreach ($reviews_admin as $review_item): ?>
                    <tr>
                        ...
                        <td><a href="<?php echo SITE_URL . 'product/?slug=' . htmlspecialchars($review_item['product_slug']); ?>" target="_blank"><?php echo htmlspecialchars($review_item['product_name']); ?></a></td> <!-- Corrected link -->
                        ...
                        <td><span class="badge bg-<?php echo $review_item['status'] == 'approved' ? 'success' : ($review_item['status'] == 'pending' ? 'warning text-dark' : 'danger'); ?>"><?php echo ucfirst($review_item['status']); ?></span></td>
                        <td class="action-buttons">
                            <form action="<?php echo SITE_URL; ?>admin/testimonials/<?php echo !empty($filter_status_admin) ? '?filter_status='.$filter_status_admin : ''; ?>" method="POST" style="display: inline;"> <!-- Corrected form action -->
