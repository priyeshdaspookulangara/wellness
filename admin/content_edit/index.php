...
switch ($page_param) {
    ...
    case 'testimonials_header':
        ...
        $breadcrumbs[] = ['name' => 'Testimonials Header'];
        break;
    case 'footer_disclaimer':
        $content_key = 'footer_disclaimer';
        $content_title = 'Footer Disclaimer Text';
        $breadcrumbs[] = ['name' => 'Footer Disclaimer'];
        break;
    default:
        $_SESSION['error_message'] = "Invalid content page specified.";
        header("Location: " . SITE_URL . "admin/"); // Corrected redirect to admin dashboard
        exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $new_content = $_POST['content_value'] ?? '';
...
<div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
    ...
    <!-- Optional: Link back to a general content management page if one exists -->
</div>
...
<form action="<?php echo SITE_URL; ?>admin/content_edit/?page=<?php echo htmlspecialchars($page_param); ?>" method="POST"> <!-- Corrected form action -->
    <div class="card">
        <div class="card-header"><?php echo htmlspecialchars($content_title); ?></div>
        <div class="card-body">
            <div class="form-group mb-3">
                <label for="content_value" class="form-label">Content:</label>
                ...
                <small class="form-text text-muted">
                    Enter the content for this section. Basic HTML may be allowed depending on how it's rendered on the frontend.
                </small>
            </div>
...
<div class="mt-4">
    <h4>Preview (Raw Text):</h4>
    <div class="card">
        <div class="card-body bg-light">
            <pre><?php echo htmlspecialchars($current_content); ?></pre>
        </div>
    </div>
</div>
